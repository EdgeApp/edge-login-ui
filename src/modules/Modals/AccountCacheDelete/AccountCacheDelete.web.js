...
export default ({
  ...
  >
    ...
    <p className={styles.text}>
      Delete <span className={styles.heavy}>{name}</span> on this device only? This will remove the 2FA and PIN access from this device.
      If 2FA is enabled on this account, you will be unable to access this account for 7 days from new devices.
    </p>
    <div className={styles.rowButtons}>
      <button className={styles.secondary} onClick={close}>Cancel</button>
      <button className={styles.primary} onClick={deleteAccount}>Delete</button>
    </div>
  </Modal>
)
